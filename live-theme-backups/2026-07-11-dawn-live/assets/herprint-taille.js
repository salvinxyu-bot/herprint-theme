/* Herprint Taille — native section engine (no iframe).
   Runs the Taille facets WebGL shader on each .hp-taille__gl canvas.
   Depends on herprint-taille-shaders.js (window.HP_VERT / HP_PRELUDE / HP_MAIN / HP_SHADERS). */
(function () {
  function initOne(canvas) {
    ...
    var entry = (window.HP_SHADERS || []).filter(function (s) { return s.key === 'taille'; })[0];
    if (!entry) return;
  ...
  function scanAll() {
    var list = document.querySelectorAll('canvas.hp-taille__gl');
    for (var i = 0; i < list.length; i++) initOne(list[i]);
  }

  window.HPTailleScan = scanAll;
